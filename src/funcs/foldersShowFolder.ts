 ...
 */
export function foldersShowFolder(
  client: CloudinaryAssetMgmtCore,
  folder: string,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.FoldersListResponse,
    | errors.ApiError
...
> {
  const input: operations.ShowFolderRequest = {
    folder: folder,
  };

  const parsed = safeParse(
    input,
    (value) => operations.ShowFolderRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.FoldersListResponse$inboundSchema),
