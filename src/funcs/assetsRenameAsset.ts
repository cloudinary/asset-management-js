...
> {
  const input: operations.RenameAssetRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.RenameAssetRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1_1/{cloud_name}/{resource_type}/rename")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.UploadResponse$inboundSchema),
