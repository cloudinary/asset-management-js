...
> {
  const input: operations.CreateFolderRequest = {
    folder: folder,
  };

  const parsed = safeParse(
    input,
    (value) => operations.CreateFolderRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateFolderResponse$inboundSchema),
