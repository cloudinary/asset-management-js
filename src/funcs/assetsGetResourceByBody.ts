 ...
 */
export function assetsGetResourceByBody(
  client: CloudinaryAssetMgmtCore,
  request: components.ResourceGetByBodyRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ResourceInternalResponse,
    | errors.ApiError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.ResourceGetByBodyRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1_1/{cloud_name}/resources/get_resource")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ResourceInternalResponse$inboundSchema),
