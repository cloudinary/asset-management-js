...
export function peopleUpdatePerson(
  ...
  Result<
    components.UpdatePersonResponse,
    | errors.ApiError
...
> {
  const input: operations.UpdatePersonRequest = {
    personId: personId,
  ...
  const parsed = safeParse(
    input,
    (value) => operations.UpdatePersonRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1_1/{cloud_name}/people/{person_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.UpdatePersonResponse$inboundSchema),
