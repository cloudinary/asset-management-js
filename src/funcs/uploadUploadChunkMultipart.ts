...
export enum UploadChunkMultipartAcceptEnum {
 ...
 */
export function uploadUploadChunkMultipart(
  ...
  Result<
    operations.UploadChunkMultipartResponse,
    | errors.ApiError
...
> {
  const input: operations.UploadChunkMultipartRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.UploadChunkMultipartRequest$outboundSchema.parse(value),
  ...
  if (isBlobLike(payload.binary_upload_request.file)) {
    ...
    const contentType =
      getContentTypeFromFileName(payload.binary_upload_request.file.fileName)
      || "application/octet-stream";
    const blob = new Blob([buffer], { type: contentType });
    appendForm(body, "file", blob, payload.binary_upload_request.file.fileName);
  } else {
    const contentType =
      getContentTypeFromFileName(payload.binary_upload_request.file.fileName)
      || "application/octet-stream";
  ...
  const context = {
    ...
    operationID: "uploadChunk_multipart",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UploadChunkMultipartResponse$inboundSchema),
    M.text(302, operations.UploadChunkMultipartResponse$inboundSchema, {
      ctype: "text/html",
    }),
