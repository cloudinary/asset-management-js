...
export function assetRelationsDeleteAssetRelationsByAssetId(
  ...
  Result<
    components.AssetRelationsDeleteResponse,
    | errors.ApiError
...
> {
  const input: operations.DeleteAssetRelationsByAssetIdRequest = {
    assetId: assetId,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.DeleteAssetRelationsByAssetIdRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.unrelate_assets_by_asset_id_request, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json([200, 207], components.AssetRelationsDeleteResponse$inboundSchema),
